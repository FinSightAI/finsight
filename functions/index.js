...
// ─── GDPR: Export all user data (callable) ───────────────────────────────────
// Bundles the user's root doc + every subcollection doc under /users/{uid}
// into a single JSON payload. Client downloads as a file.
// Rate-limited: 3 calls/hour (uses existing _rateLimit helper if present).
exports.exportUserData = functions
    ...
    .https.onCall(async (data, context) => {
        ...
        // Light rate-limit using the same Firestore-based limiter as login alerts.
        try {
            if (typeof _rateLimit === 'function') {
                const ok = await _rateLimit('gdprExport', uid, 3);
                if (!ok) {
                    throw new functions.https.HttpsError('resource-exhausted', 'Too many export requests. Try again in an hour.');
                }
            }
        } catch (e) {
            if (e && e.code === 'resource-exhausted') throw e;
            // limiter helper not available — proceed silently
        }

        const userRef = db.collection('users').doc(uid);
        const userSnap = await userRef.get();
        const root = userSnap.exists ? userSnap.data() : null;

        // Walk all subcollections under /users/{uid}/**
        ...
        for (const sub of subcollections) {
            try {
                ...
                subs[sub.id] = [];
                docs.forEach(d => {
                    subs[sub.id].push({ id: d.id, data: d.data() });
                });
            } catch (err) {
                subs[sub.id] = { error: String(err && err.message || err) };
            }
        }

        // Strip sensitive admin-only fields if present
        if (root) {
            delete root.knownDevices;          // device fingerprints
            delete root.lastFailedLoginAt;
        }
...
// ─── GDPR: Delete the user's account and all data (callable) ─────────────────
// Recursively deletes /users/{uid}/** then removes the Firebase Auth user.
// Client should sign out and redirect to landing after a successful response.
exports.deleteUserAccount = functions
    ...
    .https.onCall(async (data, context) => {
        ...
        // Require an explicit confirm flag from the client — defense in depth.
        if (!data || data.confirm !== 'DELETE') {
            throw new functions.https.HttpsError(
                'failed-precondition',
                'Missing confirmation. Pass { confirm: "DELETE" }.'
            );
        }

        const userRef = db.collection('users').doc(uid);

        // Recursively delete every subcollection doc (in batches of 400).
        ...
        try {
            ...
            console.error('deleteUserAccount: firestore delete failed', err);
        ...
        try {
            ...
            console.error('deleteUserAccount: auth delete failed', err);
            throw new functions.https.HttpsError('internal', 'Failed to delete auth account');
        }

        console.log(`🗑️  GDPR account deletion completed for ${uid}`);
        return { ok: true };
    });
