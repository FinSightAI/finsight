...
(function () {
    ...
    // Each item: { icon, label, file, category } or { icon, label, submenu: [...] }
    const NAV = [
        ...
        { icon: '📈', label: 'מניות',                file: prefix + 'stocks.html',            category: 'stocks' },
        { icon: '📊', label: 'אנליטיקת תיק',          file: prefix + 'stock-analytics.html',   category: 'stock-analytics' },
        {
            ...
            submenu: [
                ...
                { label: '⚖️ השוואת מוצרים', file: prefix + 'compare-funds.html',   category: 'compare-funds' },
            ]
        },
        ...
        { icon: '🎯', label: 'אופטימיזטור פנסיה',     file: prefix + 'pension-optimizer.html', category: 'pension-optimizer' },
        { icon: '🧮', label: 'מחשבון פנסיה',          file: prefix + 'pension-calc.html',      category: 'pension-calc' },
        { icon: '📋', label: 'דוחות',                 file: prefix + 'reports.html',           category: 'reports' },
        { icon: '📅', label: 'לוח שנה פיננסי',        file: prefix + 'calendar.html',          category: 'calendar' },
        { icon: '👨‍👩‍👧‍👦', label: 'דשבורד משפחתי',     file: prefix + 'family.html',            category: 'family' },
    ...
    function buildItem(item) {
        if (item.submenu) {
            ...
            const subItems = item.submenu.map(s => {
                ...
                return `<li class="nav-item"><a href="${s.file}" class="nav-link${a}" data-category="${s.category}">${s.label}</a></li>`;
            }).join('');
        ...
        return `<li class="nav-item">
            <a href="${item.file}" class="nav-link${active}" data-category="${item.category || ''}">
                <span class="icon">${item.icon}</span>
                <span>${item.label}</span>
            </a>
        </li>`;
